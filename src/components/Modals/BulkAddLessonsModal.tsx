...
import { parseLessonsInput, ParsedLessonItem, getSchoolTodayUrl, getSchoolTodayRangeUrl } from '../../utils/lessonParser';
import {
  X,
  Layers,
  FileText,
...
export const BulkAddLessonsModal: React.FC<BulkAddLessonsModalProps> = ({
  ...
  const thisWeek = getSchoolTodayUrl(0);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-3 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 overflow-y-auto space-y-4 flex-1 text-slate-700">
          ...
          <div className="p-3.5 bg-gradient-to-r from-blue-50 to-indigo-50/50 rounded-xl border border-blue-200/80 space-y-2.5">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="text-xs font-bold text-slate-800">
                  Відкрити розклад у School Today
                </div>
                <div className="text-[11px] text-slate-500">
                  Оберіть потрібний період, скопіюйте таблицю (кнопкою «Копіювати» або Ctrl+C) та вставте в поле нижче
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 pt-0.5">
              <a
                ...
                title={`Поточний тиждень: ${thisWeek.startDateStr} – ${thisWeek.endDateStr}`}
                className={`px-3 py-1.5 text-xs rounded-lg border shadow-2xs flex items-center gap-1.5 transition-all ${
                  !isWeekend
                    ? 'font-bold bg-blue-600 text-white hover:bg-blue-700 border-blue-600'
                    : 'font-semibold bg-white text-slate-700 hover:text-blue-700 border-slate-200 hover:bg-blue-50/60'
                }`}
              >
                <span>Цей тиждень ({thisWeek.startDateStr} – {thisWeek.endDateStr})</span>
                <ExternalLink className={`w-3 h-3 ${!isWeekend ? 'text-blue-200' : 'text-slate-400'}`} />
              </a>
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                title={`Наступний тиждень: ${nextWeek.startDateStr} – ${nextWeek.endDateStr}`}
                className={`px-3.5 py-1.5 text-xs rounded-lg border shadow-xs flex items-center gap-1.5 transition-all ${
                  isWeekend
                    ? 'font-bold bg-blue-600 text-white hover:bg-blue-700 border-blue-600 active:scale-95 ring-2 ring-blue-200'
                    : 'font-semibold bg-white text-slate-700 hover:text-blue-700 border-slate-200 hover:bg-blue-50/60'
                }`}
              >
                <span>Наступний тиждень ({nextWeek.startDateStr} – {nextWeek.endDateStr})</span>
                <ExternalLink className={`w-3.5 h-3.5 ${isWeekend ? 'text-blue-200' : 'text-slate-400'}`} />
              </a>
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                title="Показати одразу 2 тижні у School Today"
                className="px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:text-indigo-900 bg-indigo-50/80 hover:bg-indigo-100/80 border border-indigo-200 rounded-lg shadow-2xs flex items-center gap-1.5 transition-all"
              >
                <span>2 тижні ({twoWeeks.startDateStr} – {twoWeeks.endDateStr})</span>
